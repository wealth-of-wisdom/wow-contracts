    ...
    describe("3 Stakers", () => {
        ...
        afterEach(() => {
            ...
            for (let i = 1; i <= totalPools.toI32(); i++) {
                ...
                const totalIsolatedFlexiShares = isolatedFlexiStakerShares[i - 1].times(BigInt.fromI32(stakersCount));

                assert.fieldEquals("Pool", ids[i], "totalFlexiSharesAmount", totalFlexiShares.toString());
                assert.fieldEquals("Pool", ids[i], "isolatedFlexiSharesAmount", totalIsolatedFlexiShares.toString());
            }
        });
