...
contract NftSale_MintGenesisNft_Unit_Test is Nft_Unit_Test {
    ...
    function test_mintGenesisNft_Creates3NewNfts() external {
        ...
        assertGenesisNft(nftContract.getNftData(NFT_TOKEN_ID_0), LEVEL_1);
        assertGenesisNft(nftContract.getNftData(NFT_TOKEN_ID_1), LEVEL_2);
        assertGenesisNft(nftContract.getNftData(NFT_TOKEN_ID_2), LEVEL_3);
    }
    ...
    function test_mintGenesisNft_Mints3Nft() external {
        ...
        assertEq(nftContract.balanceOf(alice), 2, "Alice did not receive nft");
        assertEq(nftContract.balanceOf(bob), 1, "Bob did not receive nft");
        assertEq(
            nftContract.ownerOf(NFT_TOKEN_ID_0),
            alice,
            "NFT not minted to correct address"
